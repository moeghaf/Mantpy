...
def _squidpy_image_features(
    ...
    *,
    ...
    # spot-style ``uns['spatial']`` metadata or recropping the source image.
...
def segment_cells(
    ...
    *,
    ...
    """Segment cells from a spatial proteomics image using deep learning.
...
def segment_cells_tiled(
    ...
    *,
    ...
    """Tile-based Cellpose segmentation for large images.

    The historical channel/mask call remains unchanged. ``output='anndata'``
    emits cell centroids and areas directly while tiles stream through the
    model; it never allocates a whole-slide label mask.

    Parameters
    ----------
    img
        Channel-first image stack, 2-D nuclear image,
    ...
    if output == "mask" and downsample == 1 and not isinstance(img, _IC):
