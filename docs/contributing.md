...
Mantpy uses Hatch-VCS, so the package version is derived from Git metadata and
must not be edited in `pyproject.toml`. Choose a version that follows
[Semantic Versioning][semver], in brief
...
Before releasing, update the changelog, run the complete test and build checks,
and confirm that the reviewed commit has a clean working tree. Push that exact
commit, navigate to the project's GitHub Releases page, specify `vX.Y.Z` as the
tag name, and create the release from the reviewed commit.
For more information, see [managing GitHub releases][].
Publishing the GitHub release creates the Git tag and triggers the trusted-publishing
workflow that builds and uploads the release to [PyPI][].
